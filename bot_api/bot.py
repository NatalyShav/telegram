...
from config import TOKEN, API_KEY_KURS
import sqlite3
import aiohttp
import logging
import requests
...
async def exchange_rates(message: Message):
    url = "https://v6.exchangerate-api.com/v6/13e1054a94b7ec312ec57a81/latest/USD"
    try:
        response = requests.get(url)
        data = response.json()
        if response.status_code != 200:
            await message.answer("Не удалось получить данные о курсе валюты.")
            return
        ...
        await message.answer(f"1 USD - {usd_to_rub:.2f} RUB, 1 EUR - {euro_to_rub:.2f} RUB")
    except:
